...
const categorias = ['comida', 'bebida', 'sobremesa']

const validation = (item) => {
  let msg = ''
  if (categorias.indexOf(item.categoria) == -1) msg = 'A categoria é inválida'
  if (!item.descricao) msg = 'A descrição é inválida'
  if (!item.titulo) msg = 'O titulo é inválido'
  if (!item.valor) msg = 'O valor do item é inválido'
  return msg
}
